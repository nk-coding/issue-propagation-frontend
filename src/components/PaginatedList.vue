<template>
    <div class="d-flex fill-height flex-column">
        <div class="d-flex align-center my-3 ml-3 top-bar">
            <v-text-field v-model="searchString" label="Search" class="mr-2 search-field" prepend-inner-icon="mdi-magnify">
            </v-text-field>
            <v-select v-model="currentSortField" label="Sort by" class="mx-2 sort-select" variant="outlined"
                :items="sortFields"></v-select>
            <v-btn icon class="mx-2" @click="toggleSortDirection()" variant="outlined">
                <v-icon>
        ...
        <div class="list-container flex-grow-1">
            <div v-for="(item, index) in currentItems">
                <v-divider v-if="index != 0" class="mx-3" />
...
export interface ItemManager<I> {
    ...
    fetchItems(filter: string, sortField: string, sortAscending: boolean, count: number, page: number): Promise<[I[], number]>;
}
...
async function updateItems(resetPage: boolean) {
    ...
    const [items, count] = await props.itemManager.fetchItems(searchString.value, currentSortField.value, sortAscending.value, props.itemCount, currentPage.value - 1);
